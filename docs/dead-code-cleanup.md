...
1. **删功能必须连「入口 + 路由 + 页面 + 组件」一起删**。`4658a0e` 只删了设置入口，剩下的组件、页面、路由就成了半成品——而半成品里的注释（「可从设置进入」「不挂接产品导航，仅用于预览」）会**描述一个已经不存在的状态**，把后续的清理者误导成"这是有意保留的工具"。
2. **判断一个不可达文件是不是死代码，引用计数只能当初筛**：注释不可信（可能过期），唯一可靠的是 **git 历史**——`git log -S"文件名/符号"` 看它的入口是谁删的、什么时候删的、删得干不干净。
3. 项目的死代码清理**每轮只做一层**（这次是入口，上次是路由），残留会跨多个提交累积——清理时应一次性把「入口 → 路由 → 页面 → 组件 → 资源」整条链清完。
