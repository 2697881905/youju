...
v1 起**只保留华为账号一种登录方式**，「鸿蒙 openId 主账号」从产品中下线：

| 项 | 变更 |
| :--- | :--- |
| 登录方式 | 仅华为账号（AccountKit → unionID），登录即完成绑定 |
| 绑定列表 | 只展示「华为账号 · 主账号」（脱敏尾号 + 绑定时间）；P1 微信接入后再追加可解绑项 |
| P0-3 绑定华为 | **下线**。移除页面绑定入口，后端 `POST /bindings` 对 `huawei` 返回 400 |
| P0-4 解绑 | 华为账号**不可解绑**（后端 403）。解绑会置空 `User.unionID` = 用户永久失去登录能力 |

原因：生产已禁用 openId 登录、新用户 `openId` 恒为 null（鸿蒙账号是空壳条目）；
解绑/重绑华为会导致丢号或串号。详见 `design-account-binding.md` 变更记录。
...
**P0-2 绑定列表展示**
- 描述（2026-09-23 修订）：进入页面后，`GET /v1/account/bindings` 返回当前用户可用登录方式，固定顺序：华为（主账号，永驻、不可解绑）→ 其他（P1 微信）。每条展示 provider 名称、状态、脱敏尾号与绑定时间。~~鸿蒙主账号~~已下线。
- 验收标准：
  1. 至少展示 provider 名称、绑定状态（已绑定/主账号）、绑定时间（如 2026-07-18 绑定）、脱敏尾号（如 `****1234`）。
  2. 华为账号标识为「主账号」且**无**解绑入口。
  3. 页面底部说明「华为账号是你的登录方式，登录即完成绑定，不支持解绑」。

**P0-3 ~~绑定华为账号~~（2026-09-23 下线）**
- 现状：登录即绑定，页面不再提供绑定入口（`HuaweiBindButton.ets` 已删除），后端 `POST /v1/account/bindings` 对 `provider=huawei` 返回 400「华为账号即登录方式，无需重复绑定」。
- 下线原因：已登录用户若授权另一个华为账号，会覆盖 `User.unionID` 导致串号；授权同一账号则产生冗余绑定行。

**P0-4 解绑第三方账号（含二次确认 + 防锁死）**
- 描述（2026-09-23 修订）：华为账号即登录方式，**不可解绑**（后端 403，且不再置空 `User.unionID`）；仅未来接入的第三方（如微信）提供「解绑」按钮 + 二次确认 + `DELETE /v1/account/bindings/:provider`。
- 验收标准：
  1. 解绑走二次确认弹窗（明确告知后果），需用户主动点「确定」才执行。
  2. 华为账号行**无**解绑按钮（底部文案标注「登录即完成绑定，不支持解绑」）；后端对 `DELETE /huawei` 返回 403 兜底。
