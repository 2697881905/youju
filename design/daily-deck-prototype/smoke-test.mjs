...
try {
  ...
  // 45/28px 约合阈值的 58%，且 100px/s 的慢速投影很小，不该丢出
  ...
  await page.screenshot({ path: shot('2-after-diagonal.png') });

  // —— 用例 3：竖直向下甩出 → 翻页（自由拖动的核心诉求，原实现不可能做到）——
  ...
  await page.screenshot({ path: shot('4-after-upward.png') });

  if (errors.length) throw new Error('运行期有报错：\n' + errors.join('\n'));
  console.log('\n✅ 全部通过：零 pageerror；跟手、回弹、右斜/下/上三方向丢出均正常');
